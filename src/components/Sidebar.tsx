...
export function Sidebar({ onNewChat, onSelectConversation, onDeleteConversation }: SidebarProps) {
  ...
  const isSkillsHubActive = location.pathname.startsWith("/agnes-hub");
    ...
    >
      ...
      >
        <button
          ...
          className={`w-full rounded-lg transition-all text-left
                     ${collapsed ? "p-1.5 flex items-center justify-center" : "px-2 py-1.5 flex items-center gap-2"}
                     text-text-secondary hover:bg-surface-hover hover:text-text-primary`}
          title={collapsed ? userId || "Set user ID" : undefined}
        >
          <div
            className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold text-white shrink-0"
            style={{ background: "linear-gradient(315deg, #a84e1e, #e8985a)" }}
          >
            {(userId?.[0] ?? "?").toUpperCase()}
          </div>
          {!collapsed && (
            <>
              <span className="flex-1 truncate text-sm font-medium">{userId || "No user"}</span>
              <svg
                className={`w-3.5 h-3.5 text-text-tertiary transition-transform ${userMenuOpen ? "rotate-180" : ""}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={2}
        ...
        {userMenuOpen && (
          <div
            className={`absolute bottom-full mb-2 rounded-xl border border-border bg-surface shadow-lg p-3 z-10
                       ${collapsed ? "left-1.5 w-56" : "left-2 right-2"}`}
          >
            ...
            <button
              ...
              className={`w-full rounded-lg px-2 py-1.5 flex items-center gap-2 text-sm font-medium transition-all
                         ${isProfileActive
                           ? "bg-accent/10 text-accent"
                           : "text-text-secondary hover:text-text-primary hover:bg-surface-hover"
                         }`}
