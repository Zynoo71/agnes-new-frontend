...
function artifactSubtitle(a: GenerationArtifactData): string {
  ...
  if (a.kind === "html") {
    return a.code ? `${(a.code.length / 1024).toFixed(1)} KB` : "";
  }
  const r = a.results[0];
    ...
    >
      <div className="aspect-video rounded overflow-hidden bg-surface-muted flex items-center justify-center shrink-0">
        {data.kind === "report" ? (
          ...
          <HtmlThumb code={data.code} url={data.url} title={data.title} />
        ) : thumb ? (
          <img
            src={thumb}
            alt={data.title}
...
function HtmlThumb({ code, url, title }: { code: string; url: string; title: string }) {
  // Render the iframe at desktop dimensions (1280×720) so the HTML lays out
  // as it would on a real screen, then visually scale it down to the tile
  // size via container-query units. pointer-events: none lets the click fall
  // through to the parent button.
  const useDoc = !!code;
  ...
  return (
    ...
    >
      <iframe
        src={useDoc ? undefined : url || undefined}
        srcDoc={useDoc ? code : undefined}
        ...
        style={{
          ...
          transform: `scale(calc(100cqw / ${VIEWPORT_W}))`,
        }}
      />
    </div>
  );
