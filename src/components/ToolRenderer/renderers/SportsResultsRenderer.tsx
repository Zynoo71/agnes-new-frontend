...
// National teams — for World Cup / Copa America / Euro / Nations League / AFCON / etc.
// ESPN's country endpoint uses FIFA-style 3-letter codes (not strict ISO — e.g.
// ENG/SCT/WAL/NIR are the FIFA home-nation codes, ISL not ICE for Iceland).
const NATIONAL_TEAM_CODE: Record<string, string> = {
  // CONMEBOL
  ...
  "Sweden": "swe", "Denmark": "den", "Norway": "nor", "Finland": "fin",
  "Iceland": "isl", "Poland": "pol",
  "Czech Republic": "cze", "Czechia": "cze",
  "Serbia": "srb",
  "Turkey": "tur", "Türkiye": "tur", "Turkiye": "tur",
  "Austria": "aut", "Hungary": "hun", "Romania": "rou", "Bulgaria": "bul",
  "Greece": "gre", "Russia": "rus", "Ukraine": "ukr",
  "Scotland": "sct", "Wales": "wal", "Northern Ireland": "nir",
  "Republic of Ireland": "irl", "Ireland": "irl",
  "North Macedonia": "mkd",
  "Bosnia and Herzegovina": "bih", "Bosnia & Herzegovina": "bih",
  "Albania": "alb", "Slovenia": "svn", "Slovakia": "svk",
  "Montenegro": "mne", "Kosovo": "kos",
  // CAF
  "Cameroon": "cmr", "Nigeria": "nga", "Ghana": "gha", "Egypt": "egy",
  "Tunisia": "tun", "Algeria": "alg",
  "Ivory Coast": "civ", "Côte d'Ivoire": "civ", "Cote d'Ivoire": "civ",
  "South Africa": "rsa", "Senegal": "sen", "Morocco": "mar",
  // AFC
  "Japan": "jpn",
  ...
  "Iraq": "irq", "Saudi Arabia": "ksa", "Qatar": "qat",
  "Thailand": "tha", "Vietnam": "vie", "Philippines": "phi", "India": "ind",
  // AFC (extended)
  "Jordan": "jor", "Lebanon": "lbn", "Syria": "syr",
  "United Arab Emirates": "uae", "UAE": "uae",
  "Kuwait": "kuw", "Bahrain": "bhr", "Oman": "oma", "Yemen": "yem",
  "Afghanistan": "afg", "Pakistan": "pak", "Bangladesh": "ban",
  "Sri Lanka": "sri",
  "Uzbekistan": "uzb", "Tajikistan": "tjk", "Kyrgyzstan": "kgz",
  "Turkmenistan": "tkm", "Kazakhstan": "kaz",
  "Hong Kong": "hkg", "Chinese Taipei": "tpe", "Taiwan": "tpe",
  "Singapore": "sin", "Malaysia": "mas", "Indonesia": "idn",
  "Brunei": "brn", "Cambodia": "cam", "Laos": "lao", "Mongolia": "mgl",
  "Timor-Leste": "tls", "East Timor": "tls",
  // CAF (extended)
  "Angola": "ang", "Zambia": "zam", "Zimbabwe": "zim",
  "Mali": "mli", "Gambia": "gam", "Guinea-Bissau": "gnb", "Guinea": "gui",
  "Tanzania": "tan", "Kenya": "ken", "Ethiopia": "eth", "Eritrea": "eri",
  "Malawi": "mwi", "Sudan": "sud", "Rwanda": "rwa", "Botswana": "bot",
  "South Sudan": "ssd", "Chad": "cha",
  "Central African Republic": "caf", "Congo": "cgo", "DR Congo": "cgo",
  "Gabon": "gab", "Cape Verde": "cpv", "São Tomé and Príncipe": "stp",
  "Seychelles": "sey", "Lesotho": "les", "Comoros": "com",
  "Eswatini": "swz", "Swaziland": "swz",
  "Madagascar": "mad", "Burundi": "bdi", "Mauritania": "mrt",
  // OFC
  ...
  "Nicaragua": "nca", "El Salvador": "slv", "Guatemala": "gua",
  "Belize": "blz", "Dominican Republic": "dom", "Dominica": "dma",
  "Antigua and Barbuda": "atg", "Saint Lucia": "lca",
  "Saint Vincent and the Grenadines": "vin",
  "Trinidad and Tobago": "tca", "Trinidad & Tobago": "tca",
};
...
function espnNationalLogo(code: string): string {
  return `https://a.espncdn.com/i/teamlogos/countries/500/${code}.png`;
}

// Merged soccer-club lookup for the all-leagues fallback (UCL/UEL pools where
// the league string is missing). Hoisted so guessTeamLogo doesn't rebuild it
// per call.
const ALL_SOCCER_ID: Record<string, number> = {
  ...PREMIER_LEAGUE_ID, ...LA_LIGA_ID, ...SERIE_A_ID, ...BUNDESLIGA_ID, ...LIGUE_1_ID,
};
...
function guessTeamLogo(league: string | undefined, teamName: string | undefined): string | undefined {
  ...
  const id = lookupSoccer(ALL_SOCCER_ID, n);
  if (id) return espnSoccerLogo(id);

  // National teams — covers World Cup / Copa America / Euro / Nations League / AFCON.
  // SerpAPI's league string for international competitions is inconsistent ("FIFA
  // World Cup" vs "World Cup" vs missing), so match by name unconditionally —
  // no major club is named just "Brazil" / "France" / etc.
  const code = NATIONAL_TEAM_CODE[n] ?? NATIONAL_TEAM_CODE[normalizeTeam(n)];
  if (code) return espnNationalLogo(code);
...
function fetchBadge(league: string | undefined, name: string): Promise<BadgeCacheEntry> {
  ...
  const task = fetch(`https://www.thesportsdb.com/api/v1/json/3/searchteams.php?t=${encodeURIComponent(name)}`)
    ...
    .then((json: { teams?: Array<{ strTeam?: string; strTeamAlternate?: string; strBadge?: string; strLeague?: string; strSport?: string }> } | null) => {
      const teams = json?.teams ?? [];
      if (!teams.length) return null;
      // searchteams.php is partial-match + sorted by intLoved, so unmatched or
      // unknown queries return Arsenal/Real Madrid/etc. Require an exact name
      // match (against strTeam or comma-split strTeamAlternate) before trusting.
      const nameLc = name.toLowerCase();
      const exact = teams.filter((t) => {
        if ((t.strTeam ?? "").toLowerCase() === nameLc) return true;
        const alts = (t.strTeamAlternate ?? "").split(",").map((s) => s.trim().toLowerCase());
        return alts.includes(nameLc);
      });
      ...
      const match = leagueLc
        ...
        : null;
      return (match ?? exact[0]).strBadge ?? null;
    })
    .catch(() => null)
    .then((badge) => {
