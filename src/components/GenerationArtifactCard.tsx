...
import type { GenerationArtifactData, HtmlArtifactData, SlideArtifactData } from "@/stores/conversationStore";
...
export function GenerationArtifactCard({ data }: { data: GenerationArtifactData }) {
  ...
  if (data.kind === "html") return <HtmlCard data={data} />;
  return null;
}
...
function HtmlCard({ data }: { data: HtmlArtifactData }) {
  const { title, code, url } = data;
  const sizeKb = code ? (code.length / 1024).toFixed(1) : null;
  ...
  return (
    <div className="rounded-xl border border-border-light bg-surface-alt p-3.5 shadow-sm">
      <div className="flex items-center gap-2 mb-2">
        ...
        {sizeKb && (
          <span className="text-[10px] text-text-tertiary shrink-0">{sizeKb} KB</span>
        )}
        {url && (
          <a
            href={url}
      ...
      <HtmlPreviewFrame
        src={code ? undefined : url || undefined}
