...
const AGENT_TYPES = ["super", "search", "research", "slide", "design", "sheet", "chat_only"] as const;
    ...
    <div className={`rounded-[20px] border border-border bg-surface shadow-sm
      ...
      <div className="flex items-center justify-between px-3 pt-2.5 pb-0">
        <div className="flex items-center gap-1">
          {AGENT_TYPES.map((t) => {
            const customLabel = AGENT_TYPE_LABELS[t];
            return (
              <button
                key={t}
                onClick={() => {
                  ...
                  if (t !== "super") {
                    useModelStore.getState().setAlias("");
                    if (conversationId) useConversationListStore.getState().update(conversationId, { llmAlias: "" });
                  }
                }}
