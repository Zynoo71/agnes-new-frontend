...
export type ArtifactKind = "report" | "image" | "video" | "slide" | "html";
...
// `html` is intentionally absent: write_file is generic and only some calls
// produce an HtmlArtifact, so the card is appended rather than replacing.
const KIND_TO_TOOL_NAME: Partial<Record<ArtifactKind, string>> = {
  report: "write_report",
...
// Insert/replace at the most recent matching anchor; append if none. Searches
// backward so repeated tools in a turn anchor to their latest occurrence.
function insertAnchored(
  blocks: ContentBlock[],
  block: ContentBlock,
  match: (b: ContentBlock) => boolean,
  position: "before" | "replace",
): ContentBlock[] {
  for (let i = blocks.length - 1; i >= 0; i--) {
    if (match(blocks[i])) {
      const next = blocks.slice();
      next.splice(i, position === "replace" ? 1 : 0, block);
      return next;
    }
  }
  ...
  switch (event.event.case) {
    ...
    case "custom": {
      ...
      if (custom.type === "PromptEnhanced") {
        ...
        const targetKind: ArtifactKind = mediaType === "video" ? "video" : "image";
        const targetToolName = KIND_TO_TOOL_NAME[targetKind];
        ...
        // Wire often emits PromptEnhanced after generate_image has started or
        // even completed; anchor before to keep "enhance → generate" reading.
        updated.blocks = insertAnchored(
          updated.blocks,
          block,
          (b) =>
            (b.type === "ToolCallStart" && b.data.toolName === targetToolName) ||
            (b.type === "GenerationArtifact" && b.data.kind === targetKind),
          "before",
        );
        break;
      }
      ...
      if (custom.type === "GenerationArtifact") {
        ...
        if (!targetToolName) {
          ...
          updated.blocks = insertAnchored(
            updated.blocks,
            block,
            (b) => b.type === "ToolCallStart" && b.data.toolName === targetToolName,
            "replace",
          );
        }
        break;
...
export const useConversationStore = create<ConversationStore>((set, get) => {
  ...
  return {
    ...
    processEventForConv: (convId, event) => {
      ...
      set((prev) => {
        // Pending typewriter text must land in blocks before any non-text event
        // — otherwise the tool just pushed becomes the tail, and the next
        // typewriter tick spawns a new Message block AFTER it for chunks that
        // chronologically belong before.
        let messages = prev.messages;
        let pendingTextQueue = prev.pendingTextQueue;
        if (pendingTextQueue.length > 0) {
          const ours: typeof pendingTextQueue = [];
          const others: typeof pendingTextQueue = [];
          for (const seg of pendingTextQueue) {
            (seg.convId === convId ? ours : others).push(seg);
          }
          if (ours.length > 0) {
            ...
            if (last && last.role === "assistant") {
              ...
              for (const seg of ours) {
                appendOrPushBlock(updated.blocks, seg.type, seg.content);
              }
              flushed[flushed.length - 1] = updated;
              messages = flushed;
            }
            pendingTextQueue = others;
          }
        }
        return {
          messages: applyStreamEvent(messages, event, rawEvent.timestamp),
          pendingTextQueue,
          rawEvents: pushRawEvent(prev.rawEvents, rawEvent),
